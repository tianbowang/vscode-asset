...
class CommandCenter {
    ...
    async stashView(repository) {
        ...
        for (const change of stashChanges) {
            if (change.status === 1 /* Status.INDEX_ADDED */) {
                resources.push({ originalUri: undefined, modifiedUri: (0, uri_1.toGitUri)(change.uri, stash.hash) });
            }
            else if (change.status === 6 /* Status.DELETED */) {
                resources.push({ originalUri: (0, uri_1.toGitUri)(change.uri, stashParentCommit), modifiedUri: undefined });
            }
            else if (change.status === 3 /* Status.INDEX_RENAMED */) {
                resources.push({ originalUri: (0, uri_1.toGitUri)(change.originalUri, stashParentCommit), modifiedUri: (0, uri_1.toGitUri)(change.uri, stash.hash) });
            }
            else {
                resources.push({ originalUri: (0, uri_1.toGitUri)(change.uri, stashParentCommit), modifiedUri: (0, uri_1.toGitUri)(change.uri, stash.hash) });
            }
        }
    ...
    async _resolveTimelineOpenCommitCommand(item, uri, options) {
        ...
        const multiDiffSourceUri = (0, uri_1.toGitUri)(vscode_1.Uri.file(repository.root), item.ref, { scheme: 'git-commit' });
        const resources = [];
        for (const commitFile of commitFiles) {
            const commitFileUri = vscode_1.Uri.file(path.join(repository.root, commitFile));
            resources.push({ originalUri: (0, uri_1.toGitUri)(commitFileUri, item.previousRef), modifiedUri: (0, uri_1.toGitUri)(commitFileUri, item.ref) });
        }
        return {
    ...
    async viewCommit(repository, historyItem) {
        if (!repository || !historyItem) {
            return;
        }
        const commit = await repository.getCommit(historyItem.id);
        const title = `${historyItem.id.substring(0, 8)} - ${commit.message}`;
        const multiDiffSourceUri = (0, uri_1.toGitUri)(vscode_1.Uri.file(repository.root), historyItem.id, { scheme: 'git-commit' });
    ...
    async _viewChanges(repository, historyItem, multiDiffSourceUri, title) {
